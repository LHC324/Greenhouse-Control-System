...
{ /*引脚默认指向输入第一路*/
    GPIO_TypeDef *pGPIOx = DI0_GPIO_Port;
    uint16_t GPIO_Pinx = DI0_Pin;
    ...
    { /*读取外部数字引脚状态:翻转光耦的输入信号*/
        *pdata = (uint8_t)HAL_GPIO_ReadPin(pGPIOx, GPIO_Pinx) ? 0U : 1U;
        uint8_t site = pdata - wbits;
        /*只要有一个状态和上一次不同，则发出中断请求*/
        // if ((uint8_t)(history_wbits & (uint8_t)((*pdata) << site)) ^ (*pdata))
        if ((uint8_t)((history_wbits >> site) & 0x01) ^ (*pdata))
        {
//			if ((*(bool *)pd->Slave.pHandle) && pd->Slave.pHandle)
//			{
