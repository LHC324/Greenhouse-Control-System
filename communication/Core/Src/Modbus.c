...
#define SLAVE_MAX 0x0F
    ...
    // Modbus.Slave.pHandle = &Save_Flash.Param;
    /*定义迪文屏幕使用目标串口*/
    Modbus.huart = &huart1;
...
static bool Modbus_Operatex(pModbusHandle pd, uint16_t addr, uint8_t *pdata, uint8_t len)
{
    ...
    if (reg_addr < max)
    {
        ...
        {
            pDest = pdata, pSou = Get_RegAddr(pd, pd->Slave.Reg_Type, reg_addr);
            // pd->Slave.pPools->Coils[0] = 0x000;
            // pDest = (uint8_t *)Get_RegType(pd, pd->Slave.Reg_Type);
        }
        else
        {
            pDest = Get_RegAddr(pd, pd->Slave.Reg_Type, reg_addr), pSou = pdata;
        }
#endif
#if defined(USING_DEBUG)
...
static void Modus_ReportSeverId(pModbusHandle pd)
{
    if (Get_ModFunCode(pd) == ReportSeverId)
    {
